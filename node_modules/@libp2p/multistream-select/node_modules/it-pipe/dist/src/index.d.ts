...
export interface FnSource<A = any, B = any> {
    (source: A): Iterable<B>;
}
export interface FnAsyncSource<A = any, B = any> {
    (source: A): AsyncIterable<B>;
}
...
type PipeSource<A = any> = Iterable<A> | AsyncIterable<A> | FnSource<any, A> | FnAsyncSource<any, A> | Duplex<A, any, any>;
type PipeTransform<A = any, B extends Iterable<any> | AsyncIterable<any> = any> = FnSource<A, B> | FnAsyncSource<A, B> | Duplex<B, A, any>;
type PipeSink<A = any, B = any> = FnSink<A, B> | FnAsyncSink<A, B> | Duplex<any, A, B>;
type PipeOutput<A> = A extends FnSink ? ReturnType<A> : A extends FnAsyncSink ? ReturnType<A> : A extends Duplex<any, any, any> ? ReturnType<A['sink']> : never;
type SingleItemPipeOutput<A> = A extends Iterable<any> ? A : A extends AsyncIterable<any> ? A : A extends FnSource ? ReturnType<A> : A extends FnAsyncSource ? ReturnType<A> : A extends Duplex<any, any, any> ? A['source'] : PipeOutput<A>;
type PipeFnInput<A> = A extends Iterable<any> ? A : A extends AsyncIterable<any> ? A : A extends FnSource ? ReturnType<A> : A extends FnAsyncSource ? ReturnType<A> : A extends Duplex<any, any, any> ? A['source'] : never;
